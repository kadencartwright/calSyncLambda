...
export const lambdaHandler = async function(event: any):Promise<String>{
    ...
    let calendarGroup = (await client.api(`/users/${userId}/calendargroups?$filter=name eq '${process.env.CALENDAR_GROUP_NAME}'`).get()).value[0]
    console.log('getting calendars in group')
    let calendars = (await client.api(`/users/${userId}/calendargroups/${calendarGroup.id}/calendars`).get()).value
    for (const calendar of calendars){
        console.log('getting calendar id')
        let calendarId = (await client.api(`/users/${userId}/calendargroups/${calendarGroup.id}/calendars/${calendar.id}`).get()).id
        eventResource.push({
