...
async function syncAllEventsInCalendar(calendarId:String,client:Client){
    ...
    let calendarName = (await client.api(`/users/${process.env.CALENDAR_OWNER_UPN}/calendars/${calendarId}`).get()).name
    //find the group
    try{    
        ...
        for (const event of events){
            ...
            for (let groupMember of groupMembers){
                ...
                if (!found){
                    attendees.push({emailAddress:{address:groupMember.mail,name:groupMember.displayName}})
                }
            }
